  ...
  */
  ...
  React.useEffect(() => {
    if (rendered && vrModeRef.current) {
      ...
      // const euler = [
      //   euler_ee_cam[0],
      //   euler_ee_cam[1] + (thumbstick_right[0] * euler_sensitivity),
      //   euler_ee_cam[2] + (thumbstick_right[1] * euler_sensitivity)
      // ]
      // const R_cam = mr.EulerToRotMat(euler, Euler_order);
      ...
      let position_diff
      if (grip_on_left){      
        position_diff = [ 0, 0, -thumbstick_left[1] * position_sensitivity]
      }
      else{
        position_diff = [0, 0, 0]
      }
      ...
      const { new_theta_body, error_code } = IK_joint_velocity_limit(T_cam, robotParams.cam, theta_body_cam, VR_Control_Mode);
  ...
  // React.useEffect(() => {
  //   if (rendered && vrModeRef.current && grip_on_left) {
  //     const position_sensitivity = 0.01
  ...
  //     const position_diff = [thumbstick_left[0] * position_sensitivity, 0, thumbstick_left[1] * position_sensitivity]
  //     const position_relative = mr.matDot(R_ee_cam, position_diff);
  ...
  // }, [rendered, vrModeRef.current, grip_on_left, thumbstick_left]);

  //   React.useEffect(() => {
  //   if (rendered && vrModeRef.current && grip_on) {
  //     const sensitivity = 0.01

  //     const new_theta_body = [
  //       theta_body_cam[0],
  //       theta_body_cam[1],
  //       theta_body_cam[2],
  //       theta_body_cam[3] + (thumbstick_right[1] * sensitivity),
  //       theta_body_cam[4] + (thumbstick_right[0] * sensitivity),
  //       theta_body_cam[5],
  //     ]
  ...
  // }, [rendered, vrModeRef.current, grip_on, thumbstick_right]);
