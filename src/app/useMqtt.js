...
export default function useMqtt({
  ...
  if (typeof window.mqttClient === 'undefined') {
    ...
    window.mqttClient.on('connect', () => {
      ...
      subscribeMQTT(MQTT_TIME_TOPIC + idtopic);
    });
  }
  ...
  const handler = (topic, message) => {
    ...
    if (topic === MQTT_DEVICE_TOPIC) {
      if (data.devId != undefined) {
        ...
        subscribeMQTT(MQTT_TIME_TOPIC + data.devId);
      }
      return;
    }

    // Publish Control Signal to Viewer
    ...
    if (!props.viewer && topic === MQTT_ROBOT_STATE_TOPIC + robotIDRef.current) {
      ...
      if (data.time != undefined) {
        // console.log("Right Arm Time:", data.time);
        // console.log("Browser Time:", Date.now());
        const offset = Date.now() - data.time;
        timeOffset(offset);
        // console.log("Time Offset (Robot - Browser):", offset, "ms");
      }
